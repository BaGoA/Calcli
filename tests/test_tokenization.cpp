...
TEST_CASE("tokenization - expression with one number", "[processing]")
{
	...
	CHECK(tokens[0].type == pinky::token::Number);
...
TEST_CASE("tokenization - expression containing numbers and operator plus", "[processing]")
{
	...
	CHECK(tokens[1].type == pinky::token::Binary_Operator);
	CHECK(tokens[1].value == "+");

	CHECK(tokens[2].type == pinky::token::Number);
...
TEST_CASE("tokenization - expression containing numbers operators and parenthesis", "[processing]")
{
	...
	CHECK(tokens[2].type == pinky::token::Left_Parenthesis);
	CHECK(tokens[2].value == "(");

	CHECK(tokens[3].type == pinky::token::Number);
	CHECK(tokens[3].value == "9.12");

	CHECK(tokens[4].type == pinky::token::Binary_Operator);
	CHECK(tokens[4].value == "-");

	CHECK(tokens[5].type == pinky::token::Number);
	CHECK(tokens[5].value == "4.314");

	CHECK(tokens[6].type == pinky::token::Right_Parenthesis);
...
TEST_CASE("tokenization - expression containing function", "[processing]")
{
	...
	CHECK(tokens[0].type == pinky::token::Function);
	CHECK(tokens[0].value == "sin");

	CHECK(tokens[1].type == pinky::token::Left_Parenthesis);
	CHECK(tokens[1].value == "(");

	CHECK(tokens[2].type == pinky::token::Number);
	CHECK(tokens[2].value == "1.57");

	CHECK(tokens[3].type == pinky::token::Right_Parenthesis);
...
TEST_CASE("tokenization - expression containing all", "[processing]")
{
	...
	CHECK(tokens[3].type == pinky::token::Binary_Operator);
	CHECK(tokens[3].value == "*");

	CHECK(tokens[4].type == pinky::token::Number);
	CHECK(tokens[4].value == "1.57");

	CHECK(tokens[5].type == pinky::token::Right_Parenthesis);
	CHECK(tokens[5].value == ")");
	
	CHECK(tokens[6].type == pinky::token::Binary_Operator);
	CHECK(tokens[6].value == "+");

	CHECK(tokens[7].type == pinky::token::Function);
	CHECK(tokens[7].value == "sqrt");

	CHECK(tokens[8].type == pinky::token::Left_Parenthesis);
	CHECK(tokens[8].value == "(");
	
	CHECK(tokens[9].type == pinky::token::Number);
	CHECK(tokens[9].value == "9.11");

	CHECK(tokens[10].type == pinky::token::Binary_Operator);
	CHECK(tokens[10].value == "-");

	CHECK(tokens[11].type == pinky::token::Number);
	CHECK(tokens[11].value == "1.54");

	CHECK(tokens[12].type == pinky::token::Right_Parenthesis);
	CHECK(tokens[12].value == ")");

	CHECK(tokens[13].type == pinky::token::Binary_Operator);
	CHECK(tokens[13].value == "^");

	CHECK(tokens[14].type == pinky::token::Number);
...
TEST_CASE("tokenization - expression containing numbers and unary operator", "[processing]")
{
	...
	CHECK(tokens[0].type == pinky::token::Unary_Operator);
	CHECK(tokens[0].value == "-");

	CHECK(tokens[1].type == pinky::token::Number);
	CHECK(tokens[1].value == "8.43");

	CHECK(tokens[2].type == pinky::token::Binary_Operator);
	CHECK(tokens[2].value == "+");
...
TEST_CASE("tokenization - expression containing numbers parenthesis unary operator", "[processing]")
{
	...
	CHECK(tokens[3].type == pinky::token::Left_Parenthesis);
	CHECK(tokens[3].value == "(");

	CHECK(tokens[4].type == pinky::token::Unary_Operator);
	CHECK(tokens[4].value == "-");
	...
	CHECK(tokens[7].type == pinky::token::Number);
	CHECK(tokens[7].value == "22.54");

	CHECK(tokens[8].type == pinky::token::Right_Parenthesis);
	CHECK(tokens[8].value == ")");
}
...
TEST_CASE("tokenization - expression containing numbers unary operator function", "[processing]")
{
	...
	CHECK(tokens[2].type == pinky::token::Unary_Operator);
	CHECK(tokens[2].value == "-");

	CHECK(tokens[3].type == pinky::token::Number);
	CHECK(tokens[3].value == "1.0");

	CHECK(tokens[4].type == pinky::token::Right_Parenthesis);
	CHECK(tokens[4].value == ")");
}
...
TEST_CASE("tokenization - expression containing function constant", "[processing]")
{
	...
	CHECK(tokens[2].type == pinky::token::Constant);
...
TEST_CASE("tokenization - expression containing operator constant", "[processing]")
{
	...
	CHECK(tokens[0].type == pinky::token::Constant);
